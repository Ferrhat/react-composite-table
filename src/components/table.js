...
class Table2 extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (!isEqual(this.state.activeFilters, nextState.activeFilters) || (!isEqual(this.props.data, nextProps.data))) {
            ...
            return true;
        } else if ((!isEqual(this.state.tableData, nextState.tableData))
    ...
    onUpdateRow(rowId) {
        if (!isEmpty(this.state.changedValues)) {
                    ...
                    <tr key={row.id} style={{height: '100%'}}>
                        {
                            this.props.columns.map((column) => {
                                ...
                                let selectOptions = column.selectOptions;
                                if (isPlainObject(selectOptions)) {
                                    selectOptions = [];
                                }
                                let selectOptionPathParts = defaultTo(column.selectOptionPath, '').split('.');

                                const columnNames = selectOptionPathParts.slice(0, -1);
                                if (isPlainObject(column.selectOptions) && columnNames.length > 0) {
                                    let newSelectOptionPathParts = [];
                                    columnNames.forEach(columnName => {
                                        const foundColumn = defaultTo(find(this.props.columns, c => c.name == columnName), {});
                                        const indexValue = get(this.state.tableData[rowIndex], foundColumn.indexProperty, '');
                                        if (foundColumn && indexValue) {
                                            newSelectOptionPathParts.push(indexValue);
                                        }
                                    });
                                    if (newSelectOptionPathParts.length > 0) {
                                        newSelectOptionPathParts.push(selectOptionPathParts[selectOptionPathParts.length-1]);
                                        selectOptions = get(column.selectOptions, newSelectOptionPathParts.join('.'), []);
                                    }
                                }
