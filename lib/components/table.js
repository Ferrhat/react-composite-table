...
import {isEmpty, isEqual, delay, get} from 'lodash';
...
class Table2 extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (this.state.activeFilters != nextState.activeFilters || (!isEqual(this.props.data, nextProps.data))) {
                    ...
                    <tr key={row.id} style={{height: '100%'}}>
                        {
                            this.props.columns.map((column) => {
                                if (!column.filterType || !column.editable) {
                                    return (
                                        <td key={column.name}>{get(row, column.value)}</td>
                                    );
                                    ...
                                    return (
                                        <EditableSelectField key={column.name}
                                                             value={column.selectOptions.find(option => option.value == optionId)}
            ...
            <div>
                ...
                <Modal show={this.state.isModalDialogActive} onHide={this.handleClose}>
                    ...
                    <Modal.Footer>
                        ...
                        {!this.state.isDeleting &&
                        <Button onClick={this.deleteRow} bsStyle="defaultBlue">Yes</Button>
                        }
                    </Modal.Footer>
                </Modal>
