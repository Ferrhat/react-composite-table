...
class Table2 extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (!isEqual(this.state.activeFilters, nextState.activeFilters) || (!isEqual(this.props.data, nextProps.data))) {
            ...
            || (this.state.changedValues != nextState.changedValues)
        ...
        if (this.state.rowUnderEdit && this.props.onUpdateRow && row.id == this.state.rowUnderEditId) {
            return (
                <ul className="list-inline table-action" style={{'whiteSpace': 'nowrap'}}>
                    <TableButton className='icon icon-save' onClick={() => this.onClickSaveRow().then(() => this.setState({rowUnderEdit: false, changedValues: {}}))}/>
    ...
    render() {
        return (
            <div className='container-fluid' style={{ border: '10px solid #7fb17f', marginTop: '10px' }}>
                {
